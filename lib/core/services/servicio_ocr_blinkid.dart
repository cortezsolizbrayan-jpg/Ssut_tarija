...
class BlinkIdOcrService {
  ...
  static void _debugLogResult(BlinkIdScanningResult result) {
    if (!kDebugMode) return;
    final doc = result.documentClassInfo;
    debugPrint(
      '[BlinkID] doc=${doc?.country}/${doc?.documentType} '
      'iso=${doc?.isoAlpha2CountryCode ?? ''}',
    );
    debugPrint(
      '[BlinkID] dob=${_formatDate(result.dateOfBirth)} '
      'issue=${_formatDate(result.dateOfIssue)} '
      'expiry=${_formatDate(result.dateOfExpiry)}',
    );

    for (final viz in _vizResults(result)) {
      debugPrint(
        '[BlinkID] viz dob=${_formatDate(viz.dateOfBirth)} '
        'doc=${_stringValue(viz.documentNumber)}',
      );
    }

    for (final barcode in _barcodeResults(result)) {
      debugPrint(
        '[BlinkID] barcode dob=${_formatDate(barcode.dateOfBirth)} '
        'doc=${barcode.documentNumber ?? ''}',
      );
      if (barcode.barcodeData?.stringData != null) {
        debugPrint(
          '[BlinkID] barcode stringData=${barcode.barcodeData!.stringData}',
        );
      }
      if (barcode.barcodeData?.rawData != null) {
        debugPrint('[BlinkID] barcode rawData=${barcode.barcodeData!.rawData}');
      }
    }
  }
  ...
  static Map<String, String> _mapResult(BlinkIdScanningResult result) {
    ...
    final ci = _pickFirstNonEmpty([
      ...
      _pickFromBarcodeExtended(
        barcodeResults,
        (extended) => extended.customerIdNumber,
      ),
      _extractCiFromBarcodeRaw(barcodeResults),
    ]);

    final fechaNacimiento = _pickFirstNonEmpty([
      _pickDate([
        result.dateOfBirth,
        ...vizResults.map((viz) => viz.dateOfBirth),
        ...barcodeResults.map((barcode) => barcode.dateOfBirth),
      ]),
      _extractDateFromBarcodeRaw(barcodeResults),
    ]);
  ...
  static String _pickFromBarcodeExtended(
    Iterable<BarcodeResult> barcodeResults,
    String? Function(BarcodeExtendedElements) getter,
  ) {
    final values = barcodeResults
        .map((barcode) => barcode.extendedElements)
        .whereType<BarcodeExtendedElements>()
        .map((extended) => _stringValueFromDynamic(getter(extended)))
        .toList();
    return _pickFirstNonEmpty(values);
  }
  ...
  static String _extractCiFromBarcodeRaw(
    Iterable<BarcodeResult> barcodeResults,
  ) {
    final candidates = <String>[];
    for (final barcode in barcodeResults) {
      final rawSources = <String>[
        if (barcode.barcodeData?.stringData != null)
          barcode.barcodeData!.stringData!,
        if (barcode.barcodeData?.rawData != null) barcode.barcodeData!.rawData!,
        if (barcode.extendedElements?.customerIdNumber != null)
          barcode.extendedElements!.customerIdNumber!,
      ];
      for (final raw in rawSources) {
        candidates.addAll(_extractCiCandidatesFromText(raw));
      }
    }
  ...
  static List<String> _extractCiCandidatesFromText(String raw) {
    final text = raw.toUpperCase();
    final candidates = <String>[];
    final labeled = RegExp(
      r'(?:CI|C\\.I\\.|NRO|NO|N\u00B0|N\u00BA|NUMERO)\\s*[:\\-]?\\s*([0-9]{5,12})',
    );
    ...
    if (candidates.isEmpty) {
      final loose = RegExp(r'\\b(\\d{6,12})\\b');
      for (final match in loose.allMatches(text)) {
        final number = match.group(1) ?? '';
        if (!_looksLikeDateNumber(number)) {
          candidates.add(number);
        }
      }
    }
  ...
  static bool _looksLikeDateNumber(String value) {
    if (value.length == 8) {
      final year = int.tryParse(value.substring(0, 4)) ?? 0;
      final month = int.tryParse(value.substring(4, 6)) ?? 0;
      final day = int.tryParse(value.substring(6, 8)) ?? 0;
  ...
  static String _extractDateFromBarcodeRaw(
    Iterable<BarcodeResult> barcodeResults,
  ) {
    final sources = <String>[];
    for (final barcode in barcodeResults) {
      if (barcode.extendedElements?.dateOfBirth != null) {
        sources.add(barcode.extendedElements!.dateOfBirth!);
      }
      if (barcode.barcodeData?.stringData != null) {
        sources.add(barcode.barcodeData!.stringData!);
      }
      if (barcode.barcodeData?.rawData != null) {
        sources.add(barcode.barcodeData!.rawData!);
      }
    }
    for (final raw in sources) {
      final parsed = _normalizeDateString(raw);
  ...
  static String _normalizeDateString(String raw) {
    ...
    // Patrones con mes en letras (ej. "1 DE ENERO DE 2024", "01 ENE 2024", "01/ENE/2024")
    final monthPatterns = [
      // 1 DE ENERO DE 2024 / 01-DE-ENERO-DE-24
      RegExp(
        r'(\d{1,2})[\s\-/\.]*DE[\s\-/\.]*([A-Z]+)[\s\-/\.]*DE[\s\-/\.]*(\d{2,4})',
      ),
      // 1 ENERO 2024 / 01-ENE-24 / 01/ENE/2024
      RegExp(
  ...
  static int? _spanishMonthToNumber(String month) {
    ...
    const months = {
      ...
      'NOVIEMBRE': 11,
      'DICIEMBRE': 12,
      // Abreviaturas comunes que suele devolver OCR/BlinkID
      'ENE': 1,
      'FEB': 2,
      'MAR': 3,
      'ABR': 4,
