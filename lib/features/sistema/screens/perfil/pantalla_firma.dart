...
class _PantallaFirmaState extends State<PantallaFirma> {
  ...
  Future<void> _saveSignature() async {
    if (_points.isEmpty) {
      PremiumAlerts.showError(context, 'Por favor, dibuje su firma antes de guardar.');
      return;
    }
    ...
    try {
      ...
      final backgroundPaint = Paint()..color = Colors.white; // Fondo blanco para transparencia si se desa
      
      // Dibujar fondo transparente
      canvas.drawRect(const Rect.fromLTWH(0, 0, 500, 300), backgroundPaint);

      final Paint paint = Paint()
        ..color = const Color(0xFF003366) // Azul tinta
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Firma Digital', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
                ...
                children: [
                  ...
                  Expanded(
                    ...
                    child: ElevatedButton.icon(
                      onPressed: (_points.isEmpty || _isSaving) ? null : _saveSignature,
                      icon: _isSaving 
