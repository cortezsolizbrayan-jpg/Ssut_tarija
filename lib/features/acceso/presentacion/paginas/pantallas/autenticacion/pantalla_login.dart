  ...
  Future<void> _onGoogleRegisterPressed() async {
    try {
      ...
      final parts = fullName
          ...
          .toList();
      final nombres = parts.length > 1
          ? parts.sublist(0, parts.length - 1).join(' ')
          : fullName;
      final apellidos = parts.length > 1 ? parts.last : '';

      await LocalStorageService.saveSessionData({
        'nombreUsuario': fullName.isNotEmpty ? fullName : (account.email),
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('No se pudo autenticar con Google: $e'),
