...
class BiometricSetupPantalla extends StatefulWidget {
  static const name = 'biometric-setup-Pantalla';
  const BiometricSetupPantalla({super.key});

  @override
  State<BiometricSetupPantalla> createState() => _BiometricSetupPantallaState();
}

class _BiometricSetupPantallaState extends State<BiometricSetupPantalla> {
      ...
      if (authenticated) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text(
                'Biometría no configurada. Podrás usar solo el PIN.',
              ),
              backgroundColor: Colors.orange,
              duration: Duration(seconds: 2),
            ),
        ...
        builder: (context, setModalState) => Container(
          ...
          child: Column(
            children: [
              ...
              const Text(
                'Usa un código de 4 dígitos que recuerdes fácilmente.',
                textAlign: TextAlign.center,
              ...
              FadeInUp(
                ...
                child: SizedBox(
                  ...
                  height: 56, // Reducido un poco para ganar espacio
                  child: ElevatedButton(
                    ...
                    style: ElevatedButton.styleFrom(
                      ...
                      elevation: 4, // Añandido elevación para que se note
                    ),
                    child: const Text(
                      'CONFIRMAR PIN',
                      style: TextStyle(
  ...
  void _startEpicIntro({required String message}) {
    ...
    Future.delayed(const Duration(milliseconds: 2500), () {
      if (mounted) _goToMainPantalla();
    });
  }

  void _goToMainPantalla() async {
    // Verificar si ya pasó por la preparación del perfil
    final session = await LocalStorageService.getSessionData();
    final perfilPreparado = session?['perfilPreparado'] == true;

    if (!mounted) return;

    if (perfilPreparado) {
      // Ya pasó por la preparación — ir directo
      context.go('/sistema/pantalla_principal');
    } else {
      // Primera vez — mostrar loader de preparación (inicializa ONNX)
      context.go('/preparando-perfil');
    }
  }
              ...
              child: Column(
                children: [
                      ...
                      child: Column(
                        children: [
                          ...
                          _buildSecurityOption(
                            ...
                            subtitle: _pinConfigured
                                ? '✓ Configurado'
                                : 'Obligatorio - 4 dígitos',
                          ...
                          _buildSecurityOption(
                            ...
                            title: 'Biometría',
                            subtitle: _biometricConfigured
                                ? '✓ Configurado'
                                : 'Opcional - Huella o Rostro',
                            onTap: (_isProcessing || _biometricConfigured)
                                ? null
                                : () => _setupBiometrics(showAnimation: true),
                  ...
                  FadeIn(
                    ...
                    child: TextButton(
                      onPressed: _pinConfigured ? null : _goToMainPantalla,
