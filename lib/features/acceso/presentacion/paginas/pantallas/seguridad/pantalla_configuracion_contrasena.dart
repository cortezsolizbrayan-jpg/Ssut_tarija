...
class PasswordSetupPantalla extends StatefulWidget {
  static const name = 'password-setup-Pantalla';
  const PasswordSetupPantalla({super.key});

  @override
  State<PasswordSetupPantalla> createState() => _PasswordSetupPantallaState();
}

class _PasswordSetupPantallaState extends State<PasswordSetupPantalla> {
  ...
  String? _validatePassword(String? value) {
    ...
    if (value.contains(' ')) return 'La contraseña no puede contener espacios en blanco';
    if (value.length < 8) return 'La contraseña debe tener al menos 8 caracteres';
    if (value.length > 64) return 'La contraseña no puede superar los 64 caracteres';
    if (!RegExp(r'[A-Z]').hasMatch(value)) return 'Debe incluir al menos una letra MAYÚSCULA';
    if (!RegExp(r'[a-z]').hasMatch(value)) return 'Debe incluir al menos una letra minúscula';
  ...
  // ── Sección izquierda: título + fortaleza + requisitos ─────────────────────
  Widget _buildLeftSection(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        ...
        FadeInLeft(
          child: Text(
            'Tu contraseña debe cumplir todos los requisitos indicados a continuación para garantizar tu seguridad.',
            style: TextStyle(color: Colors.grey[600], fontSize: 15),
          ),
        ),
        SizedBox(height: ResponsiveUtils.cardSpacing(context) * 2),
        _buildStrengthBar(),
        SizedBox(height: ResponsiveUtils.cardSpacing(context)),
        _buildRequirements(),
      ],
    ...
    return Form(
      ...
      child: Column(
        ...
        children: [
          ...
          SizedBox(height: ResponsiveUtils.cardSpacing(context) * 3),
          FadeInUp(
            child: SizedBox(
        ...
        builder: (context, orientation) {
          if (orientation == Orientation.landscape) {
            ...
            return Row(
              ...
              children: [
                // Columna izquierda: título + fortaleza + requisitos
                Expanded(
  ...
  Widget _buildStrengthBar() {
    ...
    final Color barColor = isEmpty ? colors[0] : colors[level];
    final String label = isEmpty ? '' : labels[level];
      ...
      child: Column(
        ...
        children: [
          const Text(
            ...
            style: TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: Color(0xFF1A3A5C)),
          ),
  ...
  Widget _buildPasswordField(
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(label, style: const TextStyle(color: Color(0xFF848E9C), fontSize: 13)),
