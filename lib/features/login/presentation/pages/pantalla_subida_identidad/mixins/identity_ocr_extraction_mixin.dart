...
import '../utils/text_extraction_helpers.dart';
import '../utils/image_preprocessing_helpers.dart';
...
mixin IdentityOcrExtractionMixin {
  ...
  String extractNameFromText(
      ...
      for (int i = 0; i < lines.length; i++) {
        ...
        if (RegExp(
          ...
          for (int j = i + 1; j < (i + 4).clamp(0, lines.length); j++) {
            ...
            final words = TextExtractionHelpers.extractValidWords(candidateLine);
            if (words.length >= 2 &&
    ...
    if (candidateNames.isEmpty) {
      ...
      for (var line in lines) {
        ...
        final isNotCommonWord = !TextExtractionHelpers.isCommonNonNameWord(line);
  ...
  Map<String, String> extractNamesAndSurnames(RecognizedText recognizedText) {
    ...
    for (int i = 0; i < linesFromBlocks.length; i++) {
      ...
      if (RegExp(r'^APELLIDOS?[\s:]*$').hasMatch(lineUpper) && apellidos.isEmpty) {
        for (int j = i + 1; j < linesFromBlocks.length && j < i + 4; j++) {
          final candidate = linesFromBlocks[j].trim();
          if (_isValidNameCandidate(candidate)) {
      ...
      final nomInline = RegExp(r'NOMBRES?\s*:+\s*(.+)', caseSensitive: false).firstMatch(linesFromBlocks[i]);
      if (nomInline != null && nombres.isEmpty) {
        final candidate = (nomInline.group(1) ?? '').trim();
        if (_isValidNameCandidate(candidate)) nombres = _cleanNameField(candidate);
      }
      final apeInline = RegExp(r'APELLIDOS?\s*:+\s*(.+)', caseSensitive: false).firstMatch(linesFromBlocks[i]);
      if (apeInline != null && apellidos.isEmpty) {
        final candidate = (apeInline.group(1) ?? '').trim();
        if (_isValidNameCandidate(candidate)) apellidos = _cleanNameField(candidate);
      }
    }
  ...
  bool _isValidNameCandidate(String text) {
    ...
    const blacklist = [
      'SERIE', 'SECCION', 'FECHA', 'BIO', 'PLURINAC', 'ESTADO', 'BOLIVIA',
      'DOMICILIO', 'PROFESION', 'OCUPACION', 'CIVIL', 'GRUPO', 'SANGUINEO',
      'EMISION', 'EXPIRACION', 'VALIDEZ', 'CEDULA', 'IDENTIDAD', 'NUMERO',
      'FIRMA', 'FIRMA DEL INTERESADO', 'FIRMA DEL TITULAR', 'INTERESADO', 'HUELLA',
      'TITULAR', 'PERTENECE', 'CERTIFICA', 'FOTOGRAFIA', 'IMPRESION', 'DACTILAR',
    ];
    if (blacklist.any((word) => upper.contains(word))) return false;
    if (RegExp(r'^NOMBRES?[\s:]*$').hasMatch(upper) || RegExp(r'^APELLIDOS?[\s:]*$').hasMatch(upper)) return false;
    final letterPart = cleaned.replaceAll(RegExp(r"[\s\-\.']"), '');
    if (letterPart.isEmpty) return false;
    final letterCount = letterPart.split('').where((c) => RegExp(r'[A-Za-zÁ-Úá-úÑñ]').hasMatch(c)).length;
  ...
  String detectCardSide(RecognizedText recognizedText) {
    ...
    final hasN = RegExp(r'N\s*[\u00B0\u00BA]', caseSensitive: false)
        .hasMatch(fullText);
  ...
  Future<Map<String, dynamic>> performOcrExtractionWithMlKit({
    ...
    try {
      ...
      try {
        ...
        onLog?.call('Preparando variantes de imagen para OCR (multi-intento)...');
        ...
        final frontStd =
            await ImagePreprocessingHelpers.prepareFileForOcr(frontOriginalFile);
        File? backStd;
        if (backOriginalFile != null) {
          backStd = await ImagePreprocessingHelpers.prepareFileForOcr(backOriginalFile);
        }
        variants.add({'front': frontStd, 'back': backStd});

        // Variante 2: preprocesado mejorado (escala de grises + más contraste)
        final frontEnhanced = await ImagePreprocessingHelpers.preprocessForOcrEnhanced(
          frontOriginalFile,
        );
        File? backEnhanced;
        if (backOriginalFile != null) {
          backEnhanced = await ImagePreprocessingHelpers.preprocessForOcrEnhanced(
            backOriginalFile,
          );
        }
      ...
      return {
        'success': false,
        'error': 'Error al procesar las imágenes: $e',
      };
    }
  }
  ...
  Future<Map<String, dynamic>> _runSingleMlKitAttempt({
    ...
    if (apellidosMl.isEmpty && backRecognizedText != null) {
      ...
      if ((mapBack['nombres'] ?? '').trim().isNotEmpty ||
        ...
        if (apellidosMl.isEmpty) apellidosMl = (mapBack['apellidos'] ?? '').trim();
    ...
    bool _looksWeirdName(String text) {
    ...
    if ((!_isValidNameCandidate(nombresFinal) || _looksWeirdName(nombresFinal)) &&
        _isValidNameCandidate(nativeNombres)) {
      nombresFinal = nativeNombres;
    }
    if ((!_isValidNameCandidate(apellidosFinal) || _looksWeirdName(apellidosFinal)) &&
    ...
    // Fechas y otros campos pueden venir directamente de la extracción nativa
    ...
    onLog?.call('Extracción nativa aplicada y priorizada (intento único)');
  ...
  Map<String, dynamic> _combineMlKitAttempts(
    ...
    final fechaNacFinal =
        TextExtractionHelpers.pickFirstNonEmpty(fechaNacCandidates);
    final fechaEmiFinal =
        TextExtractionHelpers.pickFirstNonEmpty(fechaEmiCandidates);
    final fechaExpFinal =
        TextExtractionHelpers.pickFirstNonEmpty(fechaExpCandidates);
  ...
  String _pickBestTextCandidate(List<String> candidates) {
    final nonEmpty =
        candidates.map((c) => c.trim()).where((c) => c.isNotEmpty).toList();
    if (nonEmpty.isEmpty) return '';
    if (nonEmpty.length == 1) return nonEmpty.first;

    double bestScore = -1;
    String best = nonEmpty.first;

    for (final cand in nonEmpty) {
      double score = 0;
      for (final other in nonEmpty) {
        if (identical(cand, other)) continue;
        score += TextExtractionHelpers.calculateTextSimilarity(cand, other);
      }
      // Bonus ligero por longitud (nombres más completos)
