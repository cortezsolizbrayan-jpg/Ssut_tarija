...
-- PostgreSQL con optimizaciones específicas - Versión corregida
...
CREATE TABLE IF NOT EXISTS documentos (
    ...
    area_actual_id INTEGER NOT NULL REFERENCES areas(id),
...
-- Ahora agregar las referencias faltantes después de crear todas las tablas
ALTER TABLE areas ADD CONSTRAINT fk_areas_creado_por FOREIGN KEY (creado_por) REFERENCES usuarios(id);
ALTER TABLE tipos_documento ADD CONSTRAINT fk_tipos_documento_creado_por FOREIGN KEY (creado_por) REFERENCES usuarios(id);
...
BEGIN
    IF TG_OP = 'UPDATE' THEN
        INSERT INTO historial_documento (
            ...
            NEW.id, COALESCE(NEW.responsable_id, 1), 'ACTUALIZACION',
...
-- Crear vistas optimizadas después de tener todas las tablas
...
UPDATE tipos_documento SET creado_por = 1 WHERE creado_por IS NULL;
